...
bool CGameState2::Init()
{
	...
	//Audio
	CAudioMgr::GetInst()->PlayMusic("Game4");

	return true;
}

void CGameState2::Update(double dDt)
{
...
void CGameState2::CreateMap()
{
	char tile;
	CObject* pItem, * enemy, * cannon;

	fstream mapfile("assets/Level2.txt", ios::in);
	...
	for (int y = 0; y < 16; y++)
	{
		for (int x = 0; x < 250; x++)
		{
			...
			else if (tile == 'P')
			{
				...
				((CLevelTransition*)pPoll)->SetStateToChange(CStateMgr::STATE_GAME3);
